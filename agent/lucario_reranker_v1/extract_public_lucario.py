"""
Extract (feature, label, group) rows from the public top-episode-index
dataset (research/episodes/**), filtered to games where the WINNER piloted
a Mega Lucario ex archetype deck (Riolu id 677 or Mega Lucario ex id 678
present in the winner's starting deck list). Same schema/logic as
bc_v2/extract_data.py's winner_has_dragapult filter, retargeted to Lucario.

Run from repo root with the project venv active:
  python3 agent/lucario_reranker_v1/extract_public_lucario.py <out_jsonl>
"""
import glob
import json
import os
...
def main():
    out_path = sys.argv[1] if len(sys.argv) > 1 else "rows_public.jsonl"
    ...
    with open(out_path, "w") as out_f:
        for fp in files:
            ...
            winner_has_lucario = False
            try:
                step0_viz = steps[0][0]["visualize"][0]["action"]
                winner_deck = step0_viz[winner_idx]
                winner_has_lucario = (MEGA_LUCARIO_EX in winner_deck) or (RIOLU in winner_deck)
            except Exception:
                pass

            if not winner_has_lucario:
                continue
            n_games_lucario += 1
    ...
    print(f"Games seen: {n_games}, Lucario-winner games: {n_games_lucario}, games with usable decisions: {n_games_used}")
