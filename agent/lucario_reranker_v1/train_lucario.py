"""
Train a Mega-Lucario-ex-specific GBDT BC model on real, deck-specific,
win-filtered replay data:
  - data/rows_public.jsonl    (public top-episode-index dataset, winner
                                piloted Riolu/Mega Lucario ex, 32 games)
  - data/rows_own_ladder.jsonl (our own real live-ladder win replays from
                                mega_lucario_v2 / bc_reranker_lucario_v1 /
                                lucario_deck_tune_v1, 100% Lucario, 40 games)

Same hyperparams/methodology as bc_dragapult_v3_combined/train_combined.py's
"real_only" mode (no self-play augmentation -- that project's own retest
found self-play augmentation didn't help and mildly hurt held-out accuracy
on real games, so it's skipped here rather than re-litigated).

Usage:
  python3 train_lucario.py <out_prefix>
"""
import json
import os
import pickle
...
def main():
    out_prefix = sys.argv[1] if len(sys.argv) > 1 else "lucario_real_only"

    public_rows = load_rows(os.path.join(DATA, "rows_public.jsonl"))
    own_rows = load_rows(os.path.join(DATA, "rows_own_ladder.jsonl"))
