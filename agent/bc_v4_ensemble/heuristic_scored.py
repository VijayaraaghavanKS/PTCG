...
bc_v4_ensemble local modification (vs the unmodified copy in bc_v2/bc_reranker_v2):
exposes the raw per-option `scores` list computed inside heuristic_agent() via
the module-level `_LAST_SCORES` global, set as a side effect right before the
function commits to its own top-K selection. Used by main.py to build a
combined-rank ensemble (heuristic rank + BC rank) instead of using this
heuristic's own argmax choice directly. Purely additive -- does not change
heuristic_agent()'s own behavior/return value at all, so this file is safe to
use standalone exactly like the unmodified original (e.g. as agent()).
"""
