...
--- bc_reranker_patched_v1's heuristic_scored.py ---
Mechanical extraction of agent/dragapult_patched_v1/main.py's PATCHED
agent() into compute_option_scores() + select_from_scores(), the exact same
split bc_reranker_v2/heuristic_scored.py performs on plain dragapult_day1.
This is NOT a copy of bc_reranker_v2/heuristic_scored.py -- it is re-derived
from dragapult_patched_v1's actual patched logic (Patch S energy-sufficiency
+ Patch D danger-retreat, see AGENT_LOG.md's "dragapult_patched_v1: surgical
heuristic patches" entry), so the BC reranker in this agent's main.py sits on
top of the bug-fixed heuristic, not the original. Behavior is byte-for-byte
identical to dragapult_patched_v1/main.py's agent() (same PATCH_ABLATE-gated
patches, defaulting to both patches ACTIVE) -- only the packaging (module-
level scores computation vs. inline select) differs, same as v2's extraction
did for the unpatched base.
"""
...
def compute_option_scores(obs: Observation) -> list[int]:
    """Refactored out of dragapult_patched_v1/main.py's patched agent()
    (see agent/dragapult_patched_v1/main.py for the untouched patched
    original) so bc_reranker_patched_v1's main.py can call the exact same,
    patched-and-proven per-option scoring logic directly and use the raw
    scores for BC-margin-gated reranking, without duplicating ~500 lines of
    tactical logic. Behavior is byte-for-byte identical to
    dragapult_patched_v1's agent() -- this is a pure extraction (same split
    bc_reranker_v2/heuristic_scored.py performs on the unpatched base),
    computing and returning the `scores` list that the original function
    built just before its final sort-and-select tail.
    """
    global pre_turn_log
...
def heuristic_agent(obs_dict: dict) -> list[int]:
    """Main Agent Function -- identical behavior to dragapult_patched_v1's
