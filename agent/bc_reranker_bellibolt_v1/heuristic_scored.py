...
"""
Iono's Deck (Bellibolt ex)
Intermediate Level
This deck aims to load up your Pokémon with as much Energy as possible to unleash a powerful Voltaic Chain.

Mechanical extraction of iono_day1/main.py's agent() into compute_option_scores()
+ select_from_scores(), same pattern as bc_reranker_v2/heuristic_scored.py did for
dragapult_day1. NOT a rewrite -- behavior is byte-for-byte identical to the
original iono_day1/main.py; see that file's git history for the untouched
original. This lets bc_reranker_bellibolt_v1's main.py call the exact same,
proven per-option scoring logic directly and use the raw scores for BC-margin-
gated reranking, without duplicating the tactical logic.

Cross-decision state note (relevant to which SelectContexts are safe to
rerank -- see main.py's RERANK_ALLOWED_CONTEXTS comment): unlike
dragapult_day1 (which has plan_a/plan_b AttackPlan globals) and
mega_lucario_ref (which has a `plan` AttackPlan global), this deck's heuristic
has NO persistent attack-plan / cross-decision state at all. The only
module-level global is `can_attack`, and it is recomputed fresh from
`select.option` contents (not from which option ends up chosen) every time
context == MAIN fires -- so overriding a later decision this turn can never
desynchronize it from anything. This makes Bellibolt's heuristic structurally
safer to rerank than Dragapult's or Mega Lucario's, though the allowlist below
is kept deliberately identical/conservative to the proven bc_reranker_v2
allowlist rather than exploiting that extra safety margin -- this is a port,
not a redesign.
"""
...
def compute_option_scores(obs: Observation) -> list[int]:
    """Pure extraction of iono_day1/main.py's agent() body (from `state = obs.current`
    down to the score computation, just before its final sort-and-select tail).
    Behavior is byte-for-byte identical to the original."""
    state = obs.current
    select = obs.select
...
def heuristic_agent(obs_dict: dict) -> list[int]:
    """Main Agent Function -- identical behavior to the original iono_day1
