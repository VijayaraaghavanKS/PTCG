...
Max_Rod = 1110  # x0 (deck-tune v1: cut, redundant with Night_Stretcher/Boss_Orders added instead)
Energy_Retrieval = 1118  # x0 (deck-tune v1: cut, redundant with Night_Stretcher/Boss_Orders added instead)
Ultra_Ball = 1121  # ×3
Poke_Pad = 1152  # x2
Boss_Orders = 1182  # ×2
...
def agent(obs_dict: dict) -> list[int]:
    ...
    for c in my_state.hand:
        ...
        if c.id == Iono_Voltorb:
            ...
            score = 160
        elif c.id == Boss_Orders:
            # deck-tune v1 addition: real, keep-worthy card -- must not fall through
            # to the -10000 "unregistered" default, or Ultra_Ball/Canari's discard
            # step will treat it as free fodder to throw away.
            score = 150
        elif c.id == Levincia:
