        ...
        if o.type == OptionType.NUMBER:
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if card.id == Switch:
                    ...
                    # BUGFIX (was `if stadium_id == 0: score = -1`, i.e. refused to
                    # play into an EMPTY stadium slot -- verified via instrumentation
                    # this made the card's PLAY option score -1 (never chosen) in
                    # 2377/2377 offered decisions across 150 games, 100% dead weight.
                    # Correct intent: only skip replaying our OWN Gravity Mountain
                    # when it's already active (no-op); play it whenever the slot is
                    # empty or holds the opponent's stadium instead.
