...
Mechanical extraction of mega_lucario_ref/main.py's agent() into
compute_option_scores() + select_from_scores(), same pattern as
bc_reranker_v2/heuristic_scored.py did for dragapult_day1. NOT a rewrite --
behavior is byte-for-byte identical to the original mega_lucario_ref/main.py.

Cross-decision state note (relevant to which SelectContexts are safe to
rerank -- see main.py's RERANK_ALLOWED_CONTEXTS comment, and AGENT_LOG.md's
bc_v1/bc_reranker_v1 postmortem where overriding a plan-linked decision
cratered win rate to 0/6): this deck's heuristic DOES have persistent
cross-decision state, a `plan` AttackPlan global, structurally similar to
Dragapult's plan_a/plan_b -- but read MORE broadly here than in Dragapult.
Specifically: `plan.attacker`/`plan.target` are read in the SWITCH/TO_ACTIVE
context on BOTH sides -- own-side selection (`o.playerIndex == my_index`)
reads `plan.attacker` (score += 100 if this bench index matches the planned
attacker) AND opponent-side selection reads `plan.target` (the
Boss's-Orders-style forced-switch target). This is different from Dragapult,
where only the OPPONENT-side branch consumed plan_a -- own-side selection
there was pure id/energy/hp scoring with no plan dependency, which is
precisely why bc_reranker_v2's "own-side-only" restriction was sufficient to
make SWITCH/TO_ACTIVE safe there. Here it is NOT sufficient: even
restricting to own-side-only options would still let the reranker override a
choice that depends on `plan.attacker`, set by a prior MAIN-context decision
this same turn. Consequently main.py's allowlist EXCLUDES SWITCH/TO_ACTIVE
entirely for this deck (not even own-side-only) -- only SETUP_ACTIVE_POKEMON
(no plan reference at all; occurs during initial game setup, before any MAIN
decision has ever run) is kept from that group.
"""
...
def compute_option_scores(obs: Observation) -> list[int]:
    """Pure extraction of mega_lucario_ref/main.py's agent() body (from
    `state = obs.current` down to the score computation, just before its
    final sort-and-select tail). Behavior is byte-for-byte identical to the
    original."""
    state = obs.current
    select = obs.select
        ...
        if o.type == OptionType.NUMBER:
            ...
            if data.cardType == CardType.POKEMON:
                ...
                if card.id == Switch:
                    ...
                    # BUGFIX (ported from lucario_deck_tune_v1, was `if stadium_id
                    # == 0: score = -1`, i.e. refused to play into an EMPTY stadium
                    # slot -- verified via instrumentation this made the card's PLAY
                    # option score -1 (never chosen) in 2377/2377 offered decisions
                    # across 150 games, 100% dead weight. Correct intent: only skip
                    # replaying our OWN Gravity Mountain when it's already active
                    # (no-op); play it whenever the slot is empty or holds the
                    # opponent's stadium instead.
...
def select_from_scores(obs: Observation, scores: list[int]) -> list[int]:
    """The original function's final sort-and-select tail, unchanged --
    including the `ability_used` side effect (only fires when context == MAIN,
    reading the TOP-scored option, i.e. what the plain heuristic would have
    chosen). This function is only ever called for context == MAIN via the
    non-rerankable fallback path in main.py (MAIN is excluded from the
    reranker's allowlist), so this side effect always reflects a real
    heuristic-chosen action, never a BC-reranked one."""
    select = obs.select
    context = select.context
...
def heuristic_agent(obs_dict: dict) -> list[int]:
    """Main Agent Function -- identical behavior to the original
    mega_lucario_ref agent(), just re-composed from the two functions above.
