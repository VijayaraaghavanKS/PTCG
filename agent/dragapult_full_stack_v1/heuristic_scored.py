...
# --- Learned turn-value model (dragapult_plan_ranked_v1 addition) ---------
# Trained offline on real Dragapult-ex win-filtered replay turns (832 rows,
# 90 games: 59 own live-ladder wins + 31 public top-episode-index wins) to
# predict a 1-step-bootstrapped "how good is the FOLLOWING turn" value from
# the state immediately after a candidate plan's rollout. This is additive
# to (never replaces) the existing hand-tuned single-turn rollout score
# below -- it targets exactly what a single-turn myopic score cannot see:
# whether a candidate's resulting position sets up a strong follow-up turn.
# Loaded via direct file-path (not sys.path/__file__ -- see bc_reranker_v2's
# postmortem in AGENT_LOG.md for why __file__ is undefined in Kaggle's
# exec()-based sandbox).
_VALUE_MODEL = None
try:
    import json as _json
    _vm_path = "value_model.json"
    if not os.path.exists(_vm_path):
        _vm_path = "/kaggle_simulations/agent/value_model.json"
    with open(_vm_path) as _f:
        _VALUE_MODEL = _json.load(_f)
except Exception:
    _VALUE_MODEL = None

VALUE_BLEND_WEIGHT = 4000.0  # scales the learned value into the hand-tuned score's units


def _walk_value_tree(tree, x):
    node = 0
    cl, cr, feat, thr, val = (tree["children_left"], tree["children_right"],
...
def predict_turn_value(feat: list[float]) -> float:
    """Returns 0.0 (neutral, no-op) on any failure -- this must never crash
    or block the existing proven scoring path."""
    if _VALUE_MODEL is None:
        return 0.0
    try:
        total = _VALUE_MODEL["init_value"]
        lr = _VALUE_MODEL["learning_rate"]
        for tree in _VALUE_MODEL["trees"]:
            total += lr * _walk_value_tree(tree, feat)
        return total
    except Exception:
        return 0.0
...
# --- dragapult_full_stack_v1 addition: ported patches S/D/H from
# dragapult_patched_v1/v2 (see AGENT_LOG.md), gated the same way (a letter
# present in PATCH_ABLATE *disables* that patch; default = all active).
# S = energy-sufficiency (secondary attackers), D = danger-retreat,
# H = hand-growth-projected KO detection. All fail-safe to original behavior.
            ...
            if cs is not None:
                if cs.result is not None and cs.result >= 0:
                    ...
                    if _VALUE_MODEL is not None:
                        try:
                            my_prize_left = len(me.prize)
                            op_prize_left = len(op.prize)
                            my_bench_n = len(list(me.bench))
                            op_bench_n = len(list(op.bench))
                            vfeat = [
                                float(my_hp), float(op_hp), float(my_prize_left), float(op_prize_left),
                                float(my_bench_n), float(op_bench_n), float(my_en),
                                float(state.turn if state.turn is not None else 0),
                                float(1 if attack_id_hit is not None else 0),
                                float(len(evolve_serials)), float(len(attach_serials)),
                                float(len(trainer_seq)), float(0),
                            ]
                            learned_value = predict_turn_value(vfeat)
                            score += learned_value * VALUE_BLEND_WEIGHT
                        except Exception:
                            learned_value = 0.0
                    ...
                    if _PLAN_DEBUG:
                        print(f"PLAN_DEBUG   cand={ci} type={opts[ci].type} score={score} "
                              ...
                              f"attack_id_hit={attack_id_hit} steps={steps} learned_value={learned_value}", file=sys.stderr)
...
def compute_option_scores(obs: Observation) -> list[int]:
    """Refactored out of dragapult_full_stack_v1/main.py's combined
    (S+D+H patches + plan-search + learned turn-value) agent() so this
    module can be reused as a heuristic_scored.py for a BC reranker layer,
    same mechanical split pattern as bc_reranker_v2/bc_reranker_patched_v1.
    """
    global pre_turn_log
...
def select_from_scores(select: "SelectData", scores: list[int]) -> list[int]:
    """The original agent()'s final sort-and-select tail, unchanged."""
    context = select.context
    output = []
    if len(scores) >= 1:
...
def heuristic_agent(obs_dict: dict) -> list[int]:
    """Main Agent Function -- identical behavior to dragapult_full_stack_v1's
