...
"""
Dragapult ex Deck - v3 (deeper-search research upgrade over dragapult_day1 /
dragapult_v2)
Advanced Level
...
upgrades on top of it, both gated to fall back to the plain heuristic on any
error/timeout:

  1. Persistent opponent-archetype belief tracking (`update_belief` /
     `archetype_belief`) - unchanged from dragapult_v2 (proven safe there,
     never measured to lose games net). A simple count-based classifier over
     three known local sparring archetypes (this deck's mirror, Iono's
     Bellibolt ex, and Mega Lucario ex) plus an "unknown" bucket, recomputed
     every turn from cards the opponent has actually revealed, feeding a
     threat_multiplier into loss-shield priority, leaf evaluation, and
     hidden-information sampling bias.

  2. A fusion-safe, genuinely multi-ply MINIMAX search layer built on the
     engine's native Search API (cg.api.search_begin/search_step/search_end).
     v2's search averaged a single generic-policy opponent reply into the
     leaf score - not adversarial, and capped at ~2 ply. v3 instead: fixes a
     small set of root candidate actions from the already-computed heuristic
     scores (fusion-safe - no per-world "vote and average" fusion), greedily
     rolls our own turn to completion, then branches over the OPPONENT's
     top-K most-plausible replies and takes the MIN (worst-case) leaf value -
     real adversarial search, not an average over one assumed friendly
     reply - optionally chaining further turn-pairs (CYCLES) for genuine
     additional plies of look-ahead via repeated search_step calls on the
     same determinized world, confirmed empirically to work (see AGENT_LOG).
     Averages that minimax value across several hidden-information
     determinizations per candidate, and only overrides the heuristic's top
     pick when the margin clears a fixed threshold.
"""
...
def compute_scores(obs: Observation) -> list[int]:
    ...
    # ---------------------------------------------------------------------
    # Angle 3 (matchup counter-strategy, no deck change): generalized
    # weakness-aware confirmed-KO detection for WHICHEVER Pokemon is
    # currently active - not just Dragapult ex's Phantom Dive (that stays
    # covered separately by main_option_proc/can_main_attack above). The
    # deck already carries Latias ex (Psychic-type, Eon Blade 200 dmg for
    # [Psychic, Psychic, Colorless]) purely as a loss-shield bench piece;
    # Mega Lucario ex (340 HP, Weakness Psychic) is one-shot by Eon Blade's
    # weakness-doubled 400 damage. Previously, if Latias ex ended up active
    # and powered (e.g. after a defensive promotion) with a lethal shot
    # available, `do_switch` (gated only on Phantom-Dive-specific
    # can_main_attack) could still retreat it back to bench for Dragapult ex
    # before it ever got to fire - discarding a free KO. This closes that
    # gap without touching the decklist or the core Dragapult game plan:
    # detect any option-menu ATTACK that would KO the opponent's active
    # Pokemon (using its real weakness/resistance vs our attacker's own
    # type), block the reflexive retreat, and score that attack above
    # ordinary setup/attach actions so it actually gets taken this turn.
    snipe_ko_attack_id = None
    if (context == SelectContext.MAIN and my_state.active and my_state.active[0] is not None
            and op_state.active and op_state.active[0] is not None):
        target = op_state.active[0]
        if not no_damage_dex(target.id):
            atk_type = card_table[my_state.active[0].id].energyType
            tdata = card_table[target.id]
            best_dmg = -1
            for o in select.option:
                ...
                if atk is None:
                    continue
                dmg = atk.damage
                if tdata.weakness == atk_type:
                    dmg *= 2
                elif tdata.resistance == atk_type:
                    dmg = max(0, dmg - 30)
                if dmg > 0 and dmg >= target.hp and dmg > best_dmg:
                    best_dmg = dmg
                    snipe_ko_attack_id = o.attackId

    do_switch = (not can_main_attack and snipe_ko_attack_id is None
                 and (bench_attacker or (active_id != Budew and field_counts[Budew] >= 1 and state.turn >= 2)))
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            score = o.attackId
            if snipe_ko_attack_id is not None and o.attackId == snipe_ko_attack_id:
                # Confirmed weakness-aware KO available right now. Deliberately
                # NOT a large score bump: attacking ends the turn, so a big
                # boost here would preempt legitimate same-turn setup plays
                # (Crispin, Boss's Orders, evolutions, energy attach, ...)
                # that score in the tens of thousands - that regressed the
                # iono_day1 matchup hard (measured -19pts at n=150) before
                # this was caught. The real fix is do_switch above (don't
                # reflexively retreat away from a lethal shot); the attack
                # option itself should still naturally wait its turn behind
                # every other positive-value action, same as the unmodified
                # heuristic, and only win once nothing better remains.
                score = o.attackId + 1000
...
# ===========================================================================
# 2. FUSION-SAFE, GENUINELY MULTI-PLY MINIMAX SEARCH LAYER (v3)
# ===========================================================================
# v2's search averaged a single generic-policy opponent response into a leaf
# score - not adversarial, and not "deeper" than 2 ply. v3 replaces that with
# a real shallow minimax, modeled directly on how the public LB~950 reference
# notebook (romanrozen_strong-start-baseline-agent-v10-lb-950) uses the same
# search_begin/search_step API: branch at the root over our own candidate
# actions (fusion-safe: fixed before any hidden-info sampling), greedy-roll
# our own turn to completion, then branch over the OPPONENT's top-K
# most-plausible replies and take the MIN (worst case for us) rather than an
# average of one sampled reply. Averaged across hidden-info determinizations
# on top of that. CYCLES controls genuine additional plies: after the first
# min-ply, if CYCLES > 1 we continue with a greedy (non-branching) rollout of
# our own next turn and another opponent min-ply, etc. - real look-ahead
# rather than more samples at a fixed depth, confirmed empirically that
# search_step can be chained indefinitely from a returned SearchState
# (verified: repeated search_step calls advance the same determinized game
# world turn after turn, including past our own subsequent MAIN decisions,
# with no engine error - see AGENT_LOG for the chaining verification probe).
#
# All constants are env-var overridable (D3_*) to allow width/depth sweeps
# from the test harness without editing this file per-run; defaults below
# are the values chosen after sweeping (see AGENT_LOG).
try:
...
# We have massive proven headroom (matches run in single-digit ms to low
# single-digit seconds against a 600s/match hard limit), so the budget here
# is set far above v2's 0.5s - real per-decision cost is worth paying for
# genuine look-ahead quality as long as the match-level budget stays safe.
SEARCH_TIME_BUDGET_S = _envf("D3_TIME_BUDGET", 1.2)
SEARCH_MIN_OPTIONS = _envi("D3_MIN_OPTIONS", 3)
SEARCH_MAX_OPTIONS = _envi("D3_MAX_OPTIONS", 16)
SEARCH_MIN_TURN = _envi("D3_MIN_TURN", 2)
N_CANDIDATES = _envi("D3_N_CAND", 4)
N_DETERMINIZATIONS = _envi("D3_N_DET", 8)
K_OPP = _envi("D3_K_OPP", 3)          # opponent replies minimized over, per cycle
CYCLES = _envi("D3_CYCLES", 2)        # 1 = 2-ply minimax (matches reference); >1 = deeper
MAX_ROLLOUT_STEPS = _envi("D3_MAX_ROLLOUT", 12)
MAX_FORCED_STEPS = _envi("D3_MAX_FORCED", 6)
OVERRIDE_MARGIN = _envf("D3_MARGIN", 350.0)

# SHIPPED DEFAULT: disabled. Every configuration tried this session (2-ply
# minimax matching the public LB~950 reference's approach: 47.3% n=150 vs
# dragapult_day1; deeper 4-ply CYCLES=2: 52.7% n=150; a much wider sweep -
# 30 determinizations, 4-way opponent branching, 3s budget: 45.3% n=150 and
# ~8s/game avg, ~10x slower for no gain) failed to clear the noise floor
# (SE ~4pt at n=150) against dragapult_day1, confirming and extending last
# session's dragapult_v2 finding with a genuinely different (adversarial
# minimax, chained multi-ply) mechanism rather than just re-trying the same
# averaging approach wider. The engineering itself is sound and safe (zero
# search errors across 15,000+ search_step calls, worst-case match timing
# stays far inside the 600s budget - see AGENT_LOG) so the code is left in
# place, correct, and easy to re-enable (set D3_SEARCH_ENABLED=1) for future
# work with more testing budget, but it is NOT shipped active by default
# since it adds real per-decision latency with no measured win-rate benefit.
SEARCH_ENABLED = os.environ.get("D3_SEARCH_ENABLED", "0") == "1"
...
def _generic_topk(obs: Observation, k: int) -> list[int]:
    """Top-k distinct single-option indices for the current MAIN-ish select,
    ranked by the generic rollout policy. Used only to pick which of the
    OPPONENT's replies to branch over during the minimax ply - branching
    over their k best-looking moves (not just their single top pick) is what
    makes this an adversarial (worst-case) evaluation rather than an average
    over one assumed friendly reply."""
    select = obs.select
    n = len(select.option) if select else 0
    if n == 0:
        return []
    try:
        scores = _generic_scores(obs)
...
def _cycle_value(sid, cur, me_i, deadline, cycles_left, threat_bonus):
    """Genuine multi-ply evaluation, continuing search_step chaining past a
    single opponent reply. `cur` is the search state right after WE already
    applied one action at a MAIN decision (root candidate, or - on a deeper
    cycle - our own greedy continuation). This:
      1. greedy-completes the rest of our current turn,
      2. advances the opponent to their next MAIN decision,
      3. branches over the opponent's top-K_OPP replies and takes the worst
         (MIN) outcome for us - real adversarial minimax, not an average,
      4. if cycles_left > 1 and the game continues, greedy-picks our own
         next action (no further root branching - keeps cost bounded) and
         recurses for another full cycle, i.e. genuine additional plies of
         look-ahead rather than more determinizations at fixed depth.
    Falls back to a static leaf evaluation at every hand-off point where the
    game ends, a side can no longer be advanced, or the time budget is hit.
    """
    sid, cur = _rollout_complete_turn(sid, cur, me_i, deadline)
    cs = cur.current
    if (cs is None or (cs.result is not None and cs.result >= 0)
            or cur.select is None or cs.yourIndex == me_i):
        return _leaf_eval(cs, me_i, threat_bonus)

    sid, cur = _advance_forced(sid, cur, 1 - me_i, deadline)
    cs = cur.current
    if (cs is None or cur.select is None or cur.select.context != SelectContext.MAIN
    ...
    op_topk = _generic_topk(cur, K_OPP)
    if not op_topk:
        return _leaf_eval(cs, me_i, threat_bonus)

    worst = None
    for op_idx in op_topk:
        if time.monotonic() > deadline:
            break
        try:
            ss2 = search_step(sid, [op_idx])
        except Exception:
            continue
        sid2, cur2 = ss2.searchId, ss2.observation
        sid2, cur2 = _rollout_complete_turn(sid2, cur2, 1 - me_i, deadline)
        cs2 = cur2.current
        if (cs2 is None or (cs2.result is not None and cs2.result >= 0)
                or cur2.select is None or cs2.yourIndex != me_i):
            val = _leaf_eval(cs2, me_i, threat_bonus)
        else:
            sid2, cur2 = _advance_forced(sid2, cur2, me_i, deadline)
            cs2 = cur2.current
            if (cycles_left > 1 and cs2 is not None
                    and not (cs2.result is not None and cs2.result >= 0)
                    and cur2.select is not None and cur2.select.context == SelectContext.MAIN
                    and cs2.yourIndex == me_i and time.monotonic() < deadline):
                choice = _generic_pick(cur2)
                if choice:
                    try:
                        ss3 = search_step(sid2, choice)
                        sid2, cur2 = ss3.searchId, ss3.observation
                        val = _cycle_value(sid2, cur2, me_i, deadline, cycles_left - 1, threat_bonus)
                    except Exception:
                        val = _leaf_eval(cur2.current, me_i, threat_bonus)
                else:
                    val = _leaf_eval(cs2, me_i, threat_bonus)
            else:
                val = _leaf_eval(cs2, me_i, threat_bonus)
        # sid2 is a distinct branch id from search_step; nothing further to
        # release here (search_end() at the top level reclaims everything
        # allocated since the matching search_begin).
...
def _search_decide(obs: Observation, base_order: list[int], base_scores: list[int]) -> int | None:
    """Fusion-safe, genuinely multi-ply minimax search override for the
    ...
    Adversarial depth: unlike v2 (which rolled the opponent forward with a
    single generic-policy reply and averaged that one leaf), each candidate
    here is scored by _cycle_value, which branches over the opponent's
    top-K_OPP replies and takes the MIN (worst case), optionally continuing
    for CYCLES full turn-pairs of real look-ahead via chained search_step
    calls - not just more determinizations at a fixed shallow depth.
    """
    global _search_available
    if not (SEARCH_ENABLED and _search_available and _SEARCH_IMPORT_OK):
        return None
    state = obs.current
    select = obs.select
        ...
        for _det in range(N_DETERMINIZATIONS):
            ...
            for idx in candidates:
                ...
                try:
                    ...
                    continue
                sid, cur = ss.searchId, ss.observation
                val = _cycle_value(sid, cur, my_index, deadline, CYCLES, threat_bonus)
                acc[idx] += val
                n_eval[idx] += 1
...
def stats_summary() -> str:
    ...
    return (
        f"[dragapult_v3] decisions={stats['decisions']} main={stats['main_decisions']} "
