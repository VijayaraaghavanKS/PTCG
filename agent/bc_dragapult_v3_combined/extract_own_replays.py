...
(pulled by pull_own_replays.py into research/own_replays/, pre-filtered to
games we won). Same schema/extraction logic as bc_v2/extract_data.py, but
the "winner" side is identified by team name, not by generic reward-argmax
(these files are already win-only, but we still need to know WHICH side is
ours to extract only our own decisions).

Run from repo root with the project venv active:
  python3 agent/bc_dragapult_v3_combined/extract_own_replays.py <own_replays_dir> <out_jsonl>
"""
import glob
import json
import os
