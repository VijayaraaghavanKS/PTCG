...
"""
Bellibolt v2 -- Iono's Bellibolt ex, Electric type.
...
# Decklist (same IDs as iono_day1)
Iono_Voltorb = 265  # x3
Iono_Tadbulb = 268  # x3
...
def heuristic_scores(obs: Observation, belief_dominant, belief_score):
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if atk is not None and op_active_hp < 10000 and atk.damage >= op_active_hp:
                # Stage 1 (immediate-win/KO): the printed base damage alone is
                # already lethal. Real damage (many attacks add "+X" text bonuses
                # on top of the printed base) can only be >= this, so this check
                # never produces a false positive -- only false negatives (attacks
                # whose damage is entirely text-driven, e.g. Voltaic Chain's
                # per-energy scaling, aren't caught here; the search layer can
                # still find those via real engine simulation).
