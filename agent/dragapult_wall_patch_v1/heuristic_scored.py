...
def wall_conservation_active(op_state, deck_count: int, hand_size: int) -> bool:
    """Anti-stall / wall-conservation gate (dragapult_wall_patch_v1).

    True when the opponent's board contains a Pokemon we already know is a
    hard, total damage immunity (reusing no_damage_dex's id set: Drednaw,
    Milotic ex, Sylveon, Crustle) AND our deck is getting worn down AND we
    still have enough in hand to act without the extra draw. Games against a
    true wall run long -- the real risk becomes decking ourselves out or
    running out of resources before finding a way around the wall (e.g. a
    non-walled prize target, or the wall rotating out), not losing a damage
    race we were never going to win by attacking into it. This only softens
    a small, explicit set of deck-thinning/energy-commitment decisions below
    -- it never blocks them outright, and it is a strict no-op (identical
    behavior to the unpatched heuristic) whenever no wall Pokemon is on the
    opponent's board, so it cannot regress non-wall matchups.
    """
    for pokemon in op_state.active:
        if pokemon is not None and no_damage_dex(pokemon.id):
            return deck_count <= 28 and hand_size >= 3
...
def compute_option_scores(obs: Observation) -> list[int]:
    ...
    # dragapult_wall_patch_v1: see wall_conservation_active() docstring. Strict
    # no-op (False) unless the opponent's board actually has a true-immunity
    # wall Pokemon on it, so this cannot change behavior in non-wall matchups.
    wall_mode = wall_conservation_active(op_state, my_state.deckCount, len(my_state.hand))
        ...
        if id == Dreepy:
            ...
            if not ignore_count or support_count == 0:
                ...
                if not can_main_attack and not bench_attacker and field_counts[Dragapult_ex] >= 1:
                    score = 55000  # genuinely still need energy -- leave this case alone
                else:
                    # We already have an attack path or a charged bench
                    # attacker this turn -- under wall_mode, searching more
                    # energy "just in case" isn't urgent. Zero (not just
                    # lowered) so it can't silently backfill as the turn's
                    # default supporter once Lillie/Brock are zeroed below --
                    # otherwise the "play *a* supporter every turn" habit just
                    # shifts to whichever deck-thinner is still highest-scored.
                    score = 0 if wall_mode else 25000
        elif id == Brock_Scouting:
            if not ignore_count or support_count == 0:
                if state.turn == 2 and field_counts[Budew] + field_counts[Latias_ex] == 0:
                    score = 50000
                else:
                    score = 0 if wall_mode else 30000
        elif id == Lillie_Determination:
            if not ignore_count or support_count == 0:
                score = 0 if wall_mode else 45000
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card.id == Dreepy:
                ...
                if deck_counts[Dreepy] + deck_counts[Drakloak] > 0:
                    # wall_mode: board is already developed enough to just wait
                    # out/play around the wall -- don't keep thinning the deck
                    # searching for more copies of pieces we don't need yet.
                    score = 3000 if (wall_mode and main_pokemon_count >= 3) else 45000
                else:
                    score = -1
