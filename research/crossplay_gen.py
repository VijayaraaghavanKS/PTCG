"""
Cross-play data generation: run games between DIFFERENT pairs of Dragapult-line
policy variants (not just mirrors), full CPU parallelism, extract decision-level
BC training rows from the WINNING side of each game only.

Rationale (see AGENT_LOG.md "Cross-play data generation" entry): earlier
same-heuristic self-play (dragapult_patched_v1 vs itself, essentially) measurably
hurt held-out accuracy -- a "distillation ceiling" (a model can't exceed its own
teacher). Cross-play between ~13 INDEPENDENTLY-EVOLVED variants, filtered to the
WINNER of each specific matchup, is closer to a weak ensemble/max-operator than
self-distillation -- worth testing honestly, not assuming it works.

Extracts features directly in-process (no raw per-game JSON ever written to
disk) to avoid the multi-GB raw-JSON cleanup problem hit in the prior self-play
run. Each worker writes its own row shard (rows_shard_<n>.jsonl); a small
per-task summary is returned through the Pool for tallying.

Usage:
  python3 research/crossplay_gen.py <out_dir> <games_per_pair> <mirror_games_per_variant> [n_workers]
"""
import itertools
import json
...
FEAT_DIR = os.path.join(ROOT, "agent", "dragapult_combined_final_v2")  # canonical 138-dim features.py

POOL_NAMES = [
    "dragapult_day1", "dragapult_v2", "dragapult_v3", "dragapult_patched_v1",
    "dragapult_patched_v2", "dragapult_plan_v1", "dragapult_plan_ranked_v1",
    "dragapult_full_stack_v1", "dragapult_wall_gust_v1", "dragapult_deck_tune_v1",
    "dragapult_combined_final_v1", "dragapult_combined_final_v2",
    "bc_dragapult_v3_combined",
]
POOL_PATHS = {name: os.path.join(ROOT, "agent", name) for name in POOL_NAMES}
POOL_PATHS["bc_dragapult_v3_combined"] = os.path.join(ROOT, "agent", "bc_dragapult_v3_combined", "reranker_test")
